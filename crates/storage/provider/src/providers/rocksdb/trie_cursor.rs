...
impl<'db> RocksDBStorageTrieCursor<'db> {
    ...
    ///
    /// Uses `total_order_seek` to bypass the prefix extractor, allowing the cursor
    /// to be reused across different addresses via `set_hashed_address()`.
    /// The `is_current_address()` check filters entries at the application level.
    pub(crate) fn new(
        provider: &'db RocksDBProvider,
        hashed_address: B256,
    ) -> Result<Self, DatabaseError> {
        let iter =
            provider.raw_iterator_for_cf_total_order(tables::StoragesTrie::NAME)?;
