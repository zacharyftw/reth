//! Contains a precompile cache backed by [`moka`] (LRU by length).

use crate::precompiles::{DynPrecompile, Precompile, PrecompileInput};
    ...
    S: Eq + Hash + std::fmt::Debug + Send + Sync + Clone + 'static,
{
    ...
    pub const fn new(
        ...
        cache: PrecompileCache<S>,
        spec_id: S,
        #[cfg(feature = "metrics")] metrics: Option<CachedPrecompileMetrics>,
    ) -> Self {
        Self {
            precompile,
            cache,
...
mod tests {
    use super::*;
    use crate::{EthEvmFactory, Evm, EvmEnv, EvmFactory};
    use revm::{
        context::TxEnv, database::EmptyDB, precompile::PrecompileOutput,
    ...
    fn test_precompile_cache_basic() {
        ...
        let cache = CachedPrecompile {
            precompile: dyn_precompile,
