    ...
    P: AccountReader,
{
...

    {
        let _span = debug_span!(
            target: "engine::tree::payload_processor::prewarm",
            "bal_to_hashed_post_state::account_loop",
        )
        .entered();
        ...
        for account_changes in bal {
            ...
            // Always fetch the account; even if we don't need the db account to construct the
            // final `Account`, doing this fills the cache.
            ...
            if !account_changes.storage_changes.is_empty() {
                storage_slots_changed += account_changes.storage_changes.len();
    ...
    tracing::debug!(
        ...
        "bal_to_hashed_post_state complete"
    );
